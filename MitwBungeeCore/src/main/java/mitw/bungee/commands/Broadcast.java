...
public class Broadcast extends Command {
	...
	public void execute(final CommandSender sender, final String[] args) {
		...
		for (final String s : Mitw.servers) {
			for (final ProxiedPlayer pl : ProxyServer.getInstance().getServerInfo(s).getPlayers()) {
				Common.tell(pl, new ComponentBuilder(Strings.replaceWithOrder(Mitw.INSTANCE.getLanguage().translate(pl, "alert")
						, Mitw.INSTANCE.getLanguage().translate(pl, serverName.toLowerCase()), ""))
	...
	private void meetupAlert(final CommandSender sender, final String serverName, final boolean team) {
		for (final String s : Mitw.servers) {
			for (final ProxiedPlayer pl : ProxyServer.getInstance().getServerInfo(s).getPlayers()) {
				Common.tell(pl, new ComponentBuilder(Strings.replaceWithOrder(Mitw.INSTANCE.getLanguage().translate(pl, "alert")
						, Mitw.INSTANCE.getLanguage().translate(pl, "meetup"), "§7(" + (team ? "§bTeam" : "§eSolo") + "§7)"))
	...
	public void UHCAlert() {
		for (final ProxiedPlayer p : BungeeCord.getInstance().getPlayers()) {
			Common.tell(p, "§7§m------------------------------------");
			for (final String string : Mitw.INSTANCE.getLanguage().translateArrays(p, "uhc")) {
	...
	public void sgAlert(final CommandSender sender, final String server) {
		for (final String s : Mitw.servers) {
			for (final ProxiedPlayer pl : ProxyServer.getInstance().getServerInfo(s).getPlayers()) {
				Common.tell(pl, new ComponentBuilder(Strings.replaceWithOrder(Mitw.INSTANCE.getLanguage().translate(pl, "alert")
						, Mitw.INSTANCE.getLanguage().translate(pl, "sg"), ""))
